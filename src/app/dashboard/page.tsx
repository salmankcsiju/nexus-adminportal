...
export default function NexusDashboard() {
  ...
  const handleApproveRequest = (request: any) => {
    ...
    setNotifications([{ id: Date.now(), company: 'Nexus', issue: `${request.name} portal is now live`, severity: 'info', time: 'Just now', read: false }, ...notifications]);
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-900 overflow-hidden relative font-sans">
        ...
        {showMobileMenu && (
          <>
            ...
            <motion.aside initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed top-0 left-0 bottom-0 w-72 bg-white z-[110] p-8 flex flex-col gap-10 lg:hidden shadow-2xl">
              <div className="flex items-center justify-between"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center shadow-2xl shadow-blue-600/20"><ShieldCheck className="w-6 h-6 text-white" /></div><h1 className="text-xl font-black tracking-tighter uppercase">Nexus</h1></div><button onClick={() => setShowMobileMenu(false)} className="p-2 text-slate-400 hover:text-slate-900"><X className="w-6 h-6" /></button></div>
      ...
      <aside className="hidden lg:flex w-72 bg-white/80 backdrop-blur-2xl border-r border-slate-200/50 p-8 flex-col gap-10 z-50">
        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center shadow-2xl shadow-blue-600/20"><ShieldCheck className="w-6 h-6 text-white" /></div><h1 className="text-xl font-black tracking-tighter uppercase">Nexus<span className="text-blue-600">_</span>Master</h1></div>
